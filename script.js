...
function showCalculator(distance) {
    ...
    calculatorContainer.innerHTML = `
        <div class="back-button">
            <button type="button" onclick="goBack()">← Back to Distance Selection</button>
        </div>
        <div class="content">
            <h1>IKD-studio</h1>
            ...
            <div class="total-time">
                ...
                <input type="text" id="total-time" inputmode="numeric" pattern="[0-9]*" x-webkit-speech>
                <button type="button" onclick="startSpeechRecognition('total-time')">🎤</button>
            </div>
            <input type="button" value="Calculate Lap Times" onclick="calculateLapTimes(${distance})" style="background-color: #333; color: #fff; padding: 10px; border: none; border-radius: 5px; cursor: pointer;">
    ...
    for (let i = 0; i < numberOfSplits; i++) {
        html += `
            <div class="split-time">
                ...
                <input type="text" id="split-time-${i + 1}" inputmode="numeric" pattern="[0-9]*" x-webkit-speech>
                <button type="button" onclick="startSpeechRecognition('split-time-${i + 1}')">🎤</button>
            </div>
        `;
    }
    return html;
...
function displayLapTimes(distance, lapTimes50m, lapTimes100m, lapTimes200m) {
    ...
    let html = '<div style="display: flex; justify-content: space-around; flex-wrap: wrap;">';
    let html50m = '<div>';
    let html100m = '<div>';
    let html200m = '<div>';
    ...
    window.scrollTo(0, 0); // Scroll to the top of the page after displaying the results
}

function goBack() {
    document.body.innerHTML = `
        <div class="logo">
            <img src="logo.png" alt="IKD Studio Logo">
        </div>
        <h2>Distance Selection</h2>
    ...
    window.scrollTo(0, 0); // Scroll to the top of the page when going back
}
